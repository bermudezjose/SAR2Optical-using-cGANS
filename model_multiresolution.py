...
class pix2pix(object):
    def __init__(self, sess, image_size=256,
        ...
        self.b_bn0 = batch_norm(name='b_bn0')
    ...
    def build_model(self):
        ...
        self.real_B = tf.placeholder(tf.float32,
                                     [self.batch_size, self.image_size, self.image_size,
                                      self.output_c_dim],
                                      name='real_B_images')
        # modificar esto para que se encuentre directamente en las arquitecturas
        self.real_A_d = tf.tanh(self.b_bn0(conv2d(self.real_A, self.input_c_dim, d_h=3, d_w=3, name='ds_conv')))  
...
#                             num_patches = 1000,
#                             show=True)
        create_dataset_case_A_multiresolution(ksize=256,
                                              dataset=self.dataset_name,
                                              mask_path=None,
                                              sar_path=self.sar_root_patch + sar_img_name,
                                              opt_path=self.opt_root_patch + opt_img_name,
