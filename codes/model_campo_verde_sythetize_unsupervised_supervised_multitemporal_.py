...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        self.g_loss = self.g_loss0 + self.g_lossl1 + 0.1*self.c_loss_Fake
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        for i in range(100):
            ...
            self.plot_loss(errC_h, legends, fig3, name=sample_dir + '/loss_classifier.png')
        ...
        # for epoch in xrange(2):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                _, summary_str, d_loss = self.sess.run([d_optim, self.d_sum, self.d_loss],
                                                       feed_dict={self.real_data: batch_images, self.dropout: True})
                self.writer.add_summary(summary_str, counter)
                # print ("d_loss -->", d_loss)

                # Update G network
                _, g_loss = self.sess.run([g_optim, self.c_loss_Fake],
                ...
                g_loss, g_lossl1, c_loss_fake, c_loss_real, out_sigmoid, labels1, feat_diff = self.sess.run([self.g_loss0, self.g_lossl1, self.c_loss_Fake, self.c_loss_Real, self.C_Fake, self.labels_classifier, self.feat_diff],
                    feed_dict={self.real_data: batch_images, self.labels_classifier: trn_labels, self.dropout: False, self.idx_samples: idx_samples, self.beta: beta})
            ...
            self.plot_loss(errC_h, legends, fig5, name=sample_dir + '/loss_classifier_generated_validation.png')
    ...
    def plot_loss(self, data, legends, fig, name):
        # n_graph = len(legends)
        colors = ['r-', 'b-', 'g-', 'y-']
        sh = np.shape(data)
        n_dim = np.shape(sh)[0]
