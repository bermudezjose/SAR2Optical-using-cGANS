...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.fake_B, self.FCN_logits = self.generator(self.real_A)
...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        self.fcn_loss, _, _ = self.cal_loss(logits=self.FCN_logits, labels=self.labels)
        ...
        self.g_loss_sup = self.g_loss + self.beta * self.fcn_loss
    ...
    def train(self, args):
        ...
        # f_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="generator/f_class_map|generator/g_d7")
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        data_trn_list_unsuper = glob.glob(datasets_root + dataset_name + 'Training/unsupervised/*.npy')
        data_trn_list_super = glob.glob(datasets_root + dataset_name + 'Training/supervised/*.npy')
        ...
        data_Dic = np.load(data_trn_list_super[6]).item()
        ...
        # for epoch in xrange(1):
            ...
            np.random.shuffle(data_trn_list_unsuper)
            batch_idxs = min(len(data_trn_list_unsuper), args.train_size) // self.batch_size

            for idx in xrange(0, batch_idxs):
            # for idx in xrange(0, 0):
                # TODO: Modify this
                img_A, img_B = load_data4FCN_CV(self,
                                                data_trn_list_unsuper,
                                                sample_index=idx,
            ...
            # # ######################## SUPERVISED TRAINING #######################
            # np.random.shuffle(data_trn_list_super)
            # batch_idxs = min(len(data_trn_list_super), args.train_size) // self.batch_size
            # for idx in xrange(0, batch_idxs):
            # # for idx in xrange(0, 0):
            #     # TODO: Modify this
            #     img_A, img_B, labels, beta = load_data4FCN_CV(self,
            ...
            #     # errG = self.g_loss.eval({self.real_data: batch_images, self.labels_classifier: g_trn_labels, self.dropout_keep_prob: 1.0})
                ...
                g_loss, g_lossl1, fcn_loss = self.sess.run([self.g_loss0, self.g_lossl1, self.fcn_loss],
                                                           feed_dict={self.real_data: batch_images, self.labels: labels, self.dropout: False, self.beta: beta})
    ...
    def cal_loss(self, logits, labels):
        ...
        loss, accuracy, prediction = self.weighted_loss(logits, labels, number_class=self.n_classes, frequency=loss_weight)
    ...
    def generator(self, image, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            self.d9 = deconv2dlayer(tf.nn.relu(d7), self.n_classes, name='g_class_map', trainable=self.isTrain)

            return self.g_output, self.d9
