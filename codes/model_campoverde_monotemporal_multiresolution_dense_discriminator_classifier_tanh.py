...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_))) \
                        + self.L1_lambda * tf.reduce_mean(tf.abs(self.real_B - self.fake_B)) + 0.00001*self.fcn_loss_fake
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        save_samples_multiresolution(self, data_list_tst, output_path=sample_dir, idx=6, epoch=0, multitemporal=False)
    ...
    def discriminator(self, img_A, img_B, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            self.h3_d_class_logits = deconv2d(tf.nn.relu(h2_d), [self.batch_size, 256, 256, self.n_classes], name='d_class')
            # h3_d is (256 x 256 x 1)
