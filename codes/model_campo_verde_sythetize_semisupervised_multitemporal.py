...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.feat_diff = tf.reduce_mean(tf.abs(self.featuresReal_ - self.featuresFake_))
...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        self.c_loss = self.beta * tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.C_logits, labels=self.labels_classifier))
        self.c_loss_Fake = self.beta * tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.C_logits_Fake, labels=self.labels_classifier))
        self.c_loss_Real = self.beta * tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.C_logits_Real, labels=self.labels_classifier))
        ...
        self.g_loss = self.g_loss0 + self.g_lossl1 + 1.0*self.c_loss_Fake
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        trn_Data, trn_Labels, labels2new_labels = create_training_samples_Classifier(image_path, labels_path)
        ...
        if isTrain:
            fig3 = plt.figure("Classifier Loss")
        # while (isTrain):
        for i in range(100):
            trn_C_loss_epoch = []
            ...
            # errC, _ = validate_generated_samplesCV_oneClass(self, data_test_list, labels2new_labels=labels2new_labels, real=False)
            errC, _ = validate_generated_samplesCV(self, data_test_list, labels2new_labels=labels2new_labels, real=False)
            print ("val c_loss--->", errC)
            ...
            fig = self.plot_loss(errC_h, legends, fig3)
            fig.savefig('loss_classifier.png', dpi=600)
        ...
        # _, _ = validate_generated_samplesCV_oneClass(self, data_test_list, labels2new_labels=labels2new_labels)
        _, _ = validate_generated_samplesCV(self, data_test_list, labels2new_labels=labels2new_labels)
        print("Training set ...")
        # _, _ = validate_generated_samplesCV_oneClass(self, data_val_list, labels2new_labels=labels2new_labels)
        _, _ = validate_generated_samplesCV(self, data_val_list, labels2new_labels=labels2new_labels)
        lossC_val_real = []
        lossC_val_fake = []
        lossC_trn_real = []
        lossC_trn_fake = []
        ...
        # feat_diff_epoch = []
        fig5 = plt.figure("Classifier loss: Real vs Fake")
        for epoch in xrange(args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                # checar esto aqui
                img_A, img_B, trn_samples, trn_labels, labels_patch, idx_samples, beta = load_data4ClassifierCV(self,
                                                                                                                data_trn_list,
                ...
                # if (beta == 0):
                    # continue
                # beta = 0
                # print ("There is no labels in this patch", beta)
                ...
                _, g_loss = self.sess.run([g_optim, self.g_loss0],
                    feed_dict={self.real_data: batch_images, self.labels_classifier: trn_labels, self.dropout: False, self.idx_samples: idx_samples, self.beta: beta})
                self.writer.add_summary(summary_str, counter)
                ...
                g_loss_total, g_loss, g_lossl1, c_loss_fake, c_loss_real, out_sigmoid, labels1, feat_diff = self.sess.run([self.g_loss, self.g_loss0, self.g_lossl1, self.c_loss_Fake, self.c_loss_Real, self.C_Fake, self.labels_classifier, self.feat_diff],
                    feed_dict={self.real_data: batch_images, self.labels_classifier: trn_labels, self.dropout: False, self.idx_samples: idx_samples, self.beta: beta})
                feat_diff_batch.append(feat_diff)
                G_gloss.append(g_loss)
                G_l1loss.append(g_lossl1)
                G_total_loss.append(g_loss_total)
                if beta == 1:
                ...
                if np.mod(idx + 1, 99) == 0:
                    ...
                    print ("gloss0 -->", np.mean(G_gloss), "glossl1 -->", np.mean(G_l1loss), "closs_fake -->", np.mean(errC_trn_fake), "closs_real -->", np.mean(errC_trn_real))
                    	# print(np.concatenate((np.argmax(out_sigmoid, axis=1).reshape(-1, 1)[:5], np.argmax(labels1, axis=1).reshape(-1, 1)[:5]), axis=1))

            print ("feat_diff", np.mean(feat_diff_batch))
            ...
            # errC_val_real, errC_val_fake = validate_generated_samplesCV_oneClass(self, data_test_list, labels2new_labels=labels2new_labels, real=False)
            errC_val_real, errC_val_fake = validate_generated_samplesCV(self, data_test_list, labels2new_labels=labels2new_labels, real=False)
            ...
            fig = self.plot_loss(errC_h, legends, fig5)
            fig.savefig(sample_dir + '/loss_classifier_generated_validation.png', dpi=600)
            # errC_trn_real, errC_trn_fake = validate_generated_samplesCV_oneClass(self, data_val_list, labels2new_labels=labels2new_labels, real=False)
            errC_trn_real, errC_trn_fake = validate_generated_samplesCV(self, data_val_list, labels2new_labels=labels2new_labels, real=False)
            print("Training real -->", errC_trn_real, "Training fake -->", errC_trn_fake)
            # print("Mean Errors-->", "D", np.mean(errorD), "G", np.mean(errorG), "Gl1", np.mean(Gl1), "Gc", np.mean(Gc))
            self.save(args.checkpoint_dir, counter)
            counter += 1
            # Save statistic by epoch
            lossG.append(np.mean(G_total_loss))
            lossG_g.append(np.mean(G_gloss))
            lossG_L1.append(np.mean(G_l1loss))
            lossC_trn_real.append(errC_trn_real)
            lossC_trn_fake.append(errC_trn_fake)
            ...
            np.save(sample_dir + '/lossC_trn_real', lossC_trn_real)
            np.save(sample_dir + '/lossC_trn_fake', lossC_trn_fake)
            np.save(sample_dir + '/lossC_val_real', lossC_val_real)
            np.save(sample_dir + '/lossC_val_fake', lossC_val_fake)
    ...
    def validate_generated_samplesCV_one_hot(self, data_val_list, labels2new_labels, real=True):
        errC_real = []
        errC_fake = []
        pred_Real = []
        pred_Fake = []
