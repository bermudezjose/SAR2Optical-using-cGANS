...
#                self.writer.add_summary(summary_str, counter)
    ...
    def generator(self, img_A, img_B=None, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            if img_B is None:
                # image = lrelu(self.g_bn_er(conv2d(img_A, tf.shape(img_A)[3], d_h=3, d_w=3, name='g_er_conv')))
                image = lrelu(self.g_bn_er(conv2d(img_A, img_A.get_shape()[-1], d_h=3, d_w=3, name='g_er_conv')))
            else:
                img_A_R = lrelu(self.g_bn_er(conv2d(img_A, img_A.get_shape()[-1], d_h=3, d_w=3, name='g_er_conv')))
                image = tf.concat([img_A_R, img_B], 3)
