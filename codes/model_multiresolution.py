...
class pix2pix(object):
    def __init__(self, sess, image_size=256,
        ...
        self.sar_root_patch = '/home/jose/DataBases/RS/SAR/Campo Verde/npy_format/'
        self.opt_root_patch = '/home/jose/DataBases/RS/SAR/Campo Verde/LANDSAT/'
    ...
    def build_model(self):
        self.real_A = tf.placeholder(tf.float32,
                                     [self.batch_size, 3*self.image_size, 3*self.image_size,
                                      self.input_c_dim],
                                      name='real_A_images')
        ...
        self.real_A_d = tf.tanh(conv2d(self.real_A, self.input_c_dim, d_h=3, d_w=3, name='ds_conv'))

        self.fake_B = self.generator(self.real_A_d)

        self.real_AB = tf.concat([self.real_A_d, self.real_B], 3)
        self.fake_AB = tf.concat([self.real_A_d, self.fake_B], 3)
        self.D, self.D_logits = self.discriminator(self.real_AB, reuse=False)
        self.D_, self.D_logits_ = self.discriminator(self.fake_AB, reuse=True)

        self.fake_B_sample = self.sampler(self.real_A_d)
...
#        data = glob.glob('/home/jose/Templates/Pix2Pix/pix2pix-tensorflow_jose/datasets/'+self.dataset_name+'/train/*.npy')
#        num_imgs = 500
#        trn_imgs = data[0:num_imgs]
#        images = [load_data(img) for img in trn_imgs] # Load files from path
#        batch_size = 1
#
#        for epoch in xrange(args.epoch):
##            np.random.shuffle(images)
#            batch_idxs = min(len(images), args.train_size) // batch_size
#
...
#            data = glob('./datasets/{}/train/*.jpg'.format(self.dataset_name))
            ...
            for idx in xrange(0, batch_idxs):
                ...
                batch_images = [load_data_multiresolution(batch_file) for batch_file in batch_files] # Load files from path
                for id_batch in range(len(batch_images)):
                    real_A = batch_images[id_batch][0].reshape(self.batch_size, 3*self.image_size, 3*self.image_size, -1)
                    real_B = batch_images[id_batch][1].reshape(self.batch_size, self.image_size, self.image_size, -1)
                    # Update D network
                    _, summary_str = self.sess.run([d_optim, self.d_sum],
    ...
    #                self.writer.add_summary(summary_str, counter)
                    ...
                    errD_fake = self.d_loss_fake.eval({ self.real_A: real_A, self.real_B: real_B })
                    errD_real = self.d_loss_real.eval({ self.real_A: real_A, self.real_B: real_B })
                    errG = self.g_loss.eval({ self.real_A: real_A, self.real_B: real_B })
...
#                             num_patches = 1000,
#                             show=True)
        create_dataset_case1_multiresolution(ksize=256,
                                            dataset=self.dataset_name,
                                            mask_path=None,
                                            sar_path=self.sar_root_patch + sar_img_name,
                                            opt_path=self.opt_root_patch + opt_img_name,
