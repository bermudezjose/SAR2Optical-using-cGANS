...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.D, self.D_logits, self.D_logits_class = self.discriminator(self.real_AB, reuse=False)
        self.D_, self.D_logits_, self.D_logits_class_ = self.discriminator(self.fake_AB, reuse=True)

        self.d_sum = tf.summary.histogram("d", self.D)
        self.d__sum = tf.summary.histogram("d_", self.D_)

        self.noise = tf.random_uniform(shape=tf.shape(self.D_logits),
                                       minval=0,
                                       maxval=0.2,
                                       dtype=tf.float32
                                       )
        ...
        self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits, labels=tf.ones_like(self.D_logits)-self.noise))
        self.d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.zeros_like(self.D_logits_)+self.noise))
        self.g_loss0 = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_logits_)-self.noise)) 
        ...
        self.fcn_vars = [var for var in t_vars if 'f_' in var.name]
        self.saver = tf.train.Saver()


    def train(self, args):
        """Train pix2pix"""
        d_optim = tf.train.AdamOptimizer(0.00001, beta1=args.beta1) \
        ...
        # /mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_synthesize_semisupervised_multitemporal/
        ...
        # val_loss = validate_FCN_CV_batchsize(self, data_test_list, labels2new_labels)
        ...
        for epoch in xrange(20):
            ...
            # for idx in xrange(0, 1):
                ...
                for img in range(self.batch_size):
                    ...
                    img_AB = load_data4FCN_CV(self,
                                              data_trn_list,
                                              sample_index=idx + img,
                                              labels2new_labels=None)
                    img_A.append(img_AB[0])
                    img_B.append(img_AB[1])
                ...
                _, summary_str = self.sess.run([d_optim, self.d_sum],
                                               feed_dict={self.real_data: batch_images, self.dropout_d: False, self.dropout_g: True})
                ...
                errD_fake = self.d_loss_fake.eval({self.real_data: batch_images, self.dropout_d: False, self.dropout_g: True})
                errD_real = self.d_loss_real.eval({self.real_data: batch_images, self.dropout_d: False, self.dropout_g: True})
                # errG = self.g_loss.eval({self.real_data: batch_images, self.labels_classifier: g_trn_labels, self.dropout_keep_prob: 1.0})

                g_loss, g_lossl1 = self.sess.run([self.g_loss0, self.g_lossl1],
                                                feed_dict={self.real_data: batch_images, self.dropout_d: False, self.dropout_g: True})
            ...
            trn_opt_generated = self.sess.run(self.fake_B,
                                              feed_dict={self.real_A: trn_img_A[:self.batch_size], self.dropout_g:True})
    ...
    def discriminator(self, image, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            self.d_d8 = deconv2dlayer(tf.nn.relu(d_d7), self.df_dim, name='d_d8') # (1*1x64)x(1x7)x5x5 + (1*1x7) = 11,207
            ...
            self.d_d9 = conv2dlayer(self.d_d8, 1, k_h=1, k_w=1, d_h=1, d_w=1, name='d_map')
            self.d_d10 = conv2dlayer(self.d_d8, self.n_classes, k_h=1, k_w=1, d_h=1, d_w=1, name='d_classes')

            return self.d_d8, self.d_d9, self.d_d10

            # h0 = lrelu(conv2d(image, self.df_dim, name='d_h0_conv'))
            # # h0 is (128 x 128 x self.df_dim)
            # h0 = tf.layers.dropout(h0, 0.25, training=self.dropout_d)
            # # h0 = tf.nn.dropout(h0, 0.75)
            # h1 = lrelu(self.d_bn1(conv2d(h0, self.df_dim*2, name='d_h1_conv')))
            # # h1 is (64 x 64 x self.df_dim*2)
            # h1 = tf.layers.dropout(h1, 0.25, training=self.dropout_d)
            # h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim*4, name='d_h2_conv')))
            # # h2 is (32x 32 x self.df_dim*4)
            # h2 = tf.layers.dropout(h2, 0.25, training=self.dropout_d)
            # h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim*8, d_h=1, d_w=1, name='d_h3_conv')))
            # # h3 is (16 x 16 x self.df_dim*8)
            # h3 = tf.layers.dropout(h3, 0.25, training=self.dropout_d)
                ...
                for col in range(0, num_cols, stride):
                    if (row+self.image_size <= num_rows) and (col+self.image_size <= num_cols):
                        ...
                        sample = self.sess.run(self.fake_B_sample,
                                               feed_dict={self.real_A: sample_image, self.dropout_d: False, self.dropout_g: True}
                                               )
                        print sample.shape
