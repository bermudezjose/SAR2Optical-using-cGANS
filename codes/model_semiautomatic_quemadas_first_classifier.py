...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        self.c_loss = self.beta * tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.C_logits, labels=self.labels_classifier))
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        data = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/quemadas_ap2_case_A_semi/train/*.npy')
        data_classifier_trn_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/quemadas_ap2_case_A_semi/Classifier/training/*.npy')
        data_classifier_val_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/quemadas_ap2_case_A_semi/Classifier/testing/*.npy')
        data_classifier_generator = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/quemadas_ap2_case_A_semi/Classifier/all_samples/*.npy')
        ...
        while (isTrain):
            trn_C_loss_epoch = []
            np.random.shuffle(data_classifier_trn_list)
            for idx in xrange(0, len(data_classifier_trn_list)):
                # Update C network
                _, trn_samples, trn_labels, _, _ = load_data4Classifier_quemadas(self,
        ...
        for epoch in xrange(args.epoch):
            ...
            np.random.shuffle(data_classifier_generator)
            ...
            for idx in xrange(0, batch_idxs):
                ...
                sar_patch, trn_samples, trn_labels, labels_patch, idx_samples = load_data4Classifier_quemadas(self,
                ...
                # g_sample = g_sample[trn_labels[:, 0] == 1]
                # g_trn_labels = trn_labels[trn_labels[:, 0] == 1]
                g_trn_labels = trn_labels

                # Update G network
                _, g_loss0, g_lossl1, c_loss = self.sess.run([g_optim, self.g_loss0, self.g_lossl1, self.c_loss],
                    feed_dict={self.real_data: batch_images, self.data_classifier: g_sample, self.labels_classifier: g_trn_labels, self.dropout_keep_prob: 1.0})
                self.writer.add_summary(summary_str, counter)
                ...
                errG = self.g_loss.eval({self.real_data: batch_images, self.data_classifier: g_sample, self.labels_classifier: g_trn_labels, self.dropout_keep_prob: 1.0})
                
                errorD.append(errD_fake + errD_real)
                errorG.append(errG)

                G.append(np.mean(g_loss0))
                Gl1.append(np.mean(g_lossl1_trn))
                Gc.append(np.mean(c_loss_g))
                if np.mod(idx, 100) == 0:
                    # print ("trn c_loss -->", c_loss_C)
                    print ("gloss0 -->", np.mean(G), "glossl1 -->", np.mean(Gl1), "closs_g -->", np.mean(Gc))
...
#                self.save(args.checkpoint_dir, counter)
    ...
    def classifier(self, features, dropout_keep_prob):
        with tf.variable_scope("classifier") as scope:
            out_layer = mlp(input=features, n_features=self.n_features, n_classes=1, n_hidden=self.n_hidden, dropout_keep_prob=dropout_keep_prob)
...
#            sar_img_name = '10_08May_2016.npy'
            ...
            np.save(self.dataset_name + '_fake_opt_semi', fake_opt)
