...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.sar = tf.concat([self.sar_t0, self.sar_t1, self.opt_t1], 3)
        self.real_A = self.sar
        self.real_B = self.opt_t0
        ##########################################################################################################
        ############################# Multitemporal using opt_t1 #################################################
        # self.sar = self.sar_t0
        # self.real_A = tf.concat([self.sar, self.opt_t1], 3)
        # self.real_B = self.opt_t0
        ##########################################################################################################
        ############################# Multitemporal using sar_t1 #################################################
        ...
        self.D, self.D_logits, self.D_Class_logits = self.discriminator(self.real_AB, reuse=False)
        self.D_, self.D_logits_, self.D_Class_logits_ = self.discriminator(self.fake_AB, reuse=True)
...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        self.fcn_loss_real, _, _ = cal_loss_Q(self, logits=self.D_Class_logits, labels=self.labels)
        self.fcn_loss_fake, _, _ = cal_loss_Q(self, logits=self.D_Class_logits_, labels=self.labels)
        self.d_loss_real = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits, labels=tf.ones_like(self.D)))
        self.d_loss_fake = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.zeros_like(self.D_)))
        self.g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=self.D_logits_, labels=tf.ones_like(self.D_))) \
                        + self.L1_lambda * tf.reduce_mean(tf.abs(self.real_B - self.fake_B)) + self.fcn_loss_fake
        ...
        self.d_loss_fcn = self.d_loss + self.fcn_loss_real
    ...
    def train(self, args):
        ...
        d_optim = tf.train.AdamOptimizer(args.lr, beta1=args.beta1) \
                          .minimize(self.d_loss_fcn, var_list=self.d_vars)
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        for epoch in xrange(counter, args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                labels = batch_images[4].reshape(self.batch_size, self.image_size, self.image_size)
                # Update D network
                _, summary_str = self.sess.run([d_optim, self.d_sum],
                                               feed_dict={self.sar_t0: sar_t0, self.opt_t0: opt_t0, self.sar_t1: sar_t1, self.opt_t1: opt_t1, self.labels: labels, self.prob_drop: 0.5})
...
#                self.writer.add_summary(summary_str, counter)
                ...
                if np.mod(idx + 1, 100) == 0:
                    # errD_fake = self.d_loss_fake.eval({ self.sar: sar_t0, self.opt: opt_t0 })
                    # errD_real = self.d_loss_real.eval({ self.sar: sar_t0, self.opt: opt_t0 })
                    errD_fake = self.d_loss_fcn.eval({ self.sar_t0: sar_t0, self.opt_t0: opt_t0, self.sar_t1: sar_t1, self.opt_t1: opt_t1, self.labels: labels })
                    errD_real = 0
                    errG = self.g_loss.eval({ self.sar_t0: sar_t0, self.opt_t0: opt_t0, self.sar_t1: sar_t1, self.opt_t1: opt_t1, self.labels: labels})
                    print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f" \
                        % (epoch, idx, batch_idxs,
                            time.time() - start_time, errD_fake+errD_real, errG))
                    fcn_loss_fake = self.fcn_loss_fake.eval({ self.sar_t0: sar_t0, self.opt_t0: opt_t0, self.sar_t1: sar_t1, self.opt_t1: opt_t1, self.labels: labels })
                    print ('fcn_loss_fake --->', fcn_loss_fake)
    ...
    def discriminator(self, image, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            ############################# DECODER ##########################################
            self.h0_d = deconv2d(tf.nn.relu(h3), [self.batch_size, 16, 16, self.gf_dim*4], name='d_h0_d')
            h0_d = tf.nn.dropout(self.d_bn_d0(self.h0_d), self.prob_drop)
            h0_d = tf.concat([h0_d, h2], 3)
            # h0_d is (32 x 32 x self.gf_dim*4)

            self.h1_d = deconv2d(tf.nn.relu(h0_d), [self.batch_size, 32, 32, self.gf_dim*2], name='d_h1_d')
            h1_d = tf.nn.dropout(self.d_bn_d1(self.h1_d), self.prob_drop)
            h1_d = tf.concat([h1_d, h1], 3)
            # h0_d is (64 x 64 x self.gf_dim*2)

            self.h2_d = deconv2d(tf.nn.relu(h1_d), [self.batch_size, 64, 64, self.gf_dim*1], name='d_h2_d')
            h2_d = self.d_bn_d2(self.h2_d)
            h2_d = tf.concat([h2_d, h0], 3)
            # h2_d is (128 x 128 x self.gf_dim)

            self.h3_d = deconv2d(tf.nn.relu(h2_d), [self.batch_size, 128, 128, 1], name='d_h3_d')
            # h3_d is (256 x 256 x 1)

            self.h3_d_class_logits = deconv2d(tf.nn.relu(h2_d), [self.batch_size, 128, 128, self.n_classes], name='d_class')
            # h3_d is (256 x 256 x 1)
