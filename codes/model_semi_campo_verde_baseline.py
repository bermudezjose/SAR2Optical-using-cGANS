...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        data_trn_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_semi/train/Training/*.npy')
        data_val_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_semi/train/Testing/*.npy')
        data_test_list= glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_semi/train/Testing/*.npy')
        ...
        fig.savefig(sample_dir + '/' + self.dataset_name + '_sample_original_tst.png', dpi=300)
        ...
        fig = self.plot_patch(trn_opt, n_fig=3)
        fig.savefig(sample_dir + '/' + self.dataset_name + '_sample_original_trn.png', dpi=300)
        ...
        for epoch in xrange(args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                # checar esto aqui
                sar_patch, opt_patch = load_data4ClassifierCV(self,
                                                              data_trn_list,
                ...
                if np.mod(idx + 1, 99) == 0:
                    ...
                    print ("gloss0 -->", np.mean(Gloss), "glossl1 -->", np.mean(Gl1loss))
            ...
            fig = self.plot_patch(tst_opt_generated, n_fig=5)
            fig.savefig(sample_dir + '/' + self.dataset_name + '_sample_tst_' + str(counter) + '_.png', dpi=300)
            ...
            fig = self.plot_patch(trn_opt_generated, n_fig=6)
            fig.savefig(sample_dir + '/' + self.dataset_name + '_sample_trn_' + str(counter) + '_.png', dpi=300)
    ...
    def validate_generated_samplesCV(self, data_val_list, labels2new_labels, real=True):
        ...
        for patch_dir in data_val_list:
            data_Dic = np.load(patch_dir).item()
            labels_patch = np.array(data_Dic['labels']).flatten()
            sar_patch = np.array(data_Dic['sar']).astype('float32')
            opt_patch = np.array(data_Dic['opt']).astype('float32')
            ...
            if real:
                pred_real, pred_fake, c_loss_fake, c_loss_real = self.sess.run([self.C_Real, self.C_Fake, self.c_loss_Fake, self.c_loss_Real],
                    feed_dict={self.real_data: batch_images, self.labels_classifier: labels_one_hot, self.dropout_keep_prob: 1.0, self.idx_samples: idx_samples})
    ...
    def classifier(self, features, dropout_keep_prob, reuse=False):
        ...
        with tf.variable_scope("classifier") as scope:
            ...
            dropout = tf.layers.dropout(inputs=hidden, rate=dropout_keep_prob)
