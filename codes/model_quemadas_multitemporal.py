...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        data_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Quemadas/Training/*.npy')
...
#                self.writer.add_summary(summary_str, counter)
            ...
            # save_samples(self, data_list, output_path=sample_dir, idx=6, epoch=epoch)
            print("ok...")
        ...
        for row in range(0, num_rows, stride):
            for col in range(0, num_cols, stride):
                if (row + self.image_size <= num_rows) and (col+self.image_size <= num_cols):
                    ...
                    sars_t1 = SAR_t1[:, row:row+self.image_size, col:col+self.image_size]
                    opts_t1 = OPT_t1[:, row:row+self.image_size, col:col+self.image_size]
