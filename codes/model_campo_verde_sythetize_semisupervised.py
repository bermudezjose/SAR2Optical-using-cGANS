...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        data_trn_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_synthesize_semisupervised/Training/*.npy')
        data_val_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_synthesize_semisupervised/Training/*.npy')
        data_test_list= glob.glob('/mnt/Data/Pix2Pix_datasets/Semi_Exp/05may2016_C01_synthesize_semisupervised/Testing/*.npy')
        ...
        fig = self.plot_patch(tst_opt, n_fig="Testing Patch")
        ...
        fig = self.plot_patch(trn_opt, n_fig="Training Patch")
        ...
        for epoch in xrange(args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                # print (beta)
                if (beta == 0):
                    print ("There is no labels in this patch")
    ...
    def discriminator(self, image, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            h0 = tf.layers.dropout(h0, 0.25, training=self.dropout)
            # h0 = tf.nn.dropout(h0, 0.75)
            h1 = lrelu(self.d_bn1(conv2d(h0, self.df_dim*2, name='d_h1_conv')))
            # h1 is (64 x 64 x self.df_dim*2)
            h1 = tf.layers.dropout(h1, 0.25, training=self.dropout)
            h2 = lrelu(self.d_bn2(conv2d(h1, self.df_dim*4, name='d_h2_conv')))
            # h2 is (32x 32 x self.df_dim*4)
            h2 = tf.layers.dropout(h2, 0.25, training=self.dropout)
            h3 = lrelu(self.d_bn3(conv2d(h2, self.df_dim*8, d_h=1, d_w=1, name='d_h3_conv')))
            # h3 is (16 x 16 x self.df_dim*8)
            h3 = tf.layers.dropout(h3, 0.25, training=self.dropout)
