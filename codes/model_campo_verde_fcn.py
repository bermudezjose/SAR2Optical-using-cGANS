...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.fake_B, self.FCN_logits_fake = self.generator(self.real_A)
        ...
        self.fake_B_sample, self.FCN_logits_sample = self.sampler(self.real_A)
...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
        ...
        # self.FCN_logits = tf.reshape(self.class_map, (-1, self.n_classes))
        # self.labels_reshaped = tf.reshape(self.labels, [-1])
        ...
        # self.fcn_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(logits=self.FCN_logits, labels=self.labels_reshaped))
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
    ...
    def fine_tuning(self, args):
        ...
        train_vars = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="generator/f_class_map|generator/g_d8|generator/g_d7")
        ...
        # for epoch in xrange(1):
            ...
            # for idx in xrange(0, 1):
                ...
                _, fcn_loss = self.sess.run([fcn_optim, self.fcn_loss_fake],
                                           feed_dict={self.real_data: batch_images, self.labels: labels, self.dropout_g: True})
                # print (fcn_loss)
    ...
    def generator(self, image, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            self.d9 = conv2dlayer(self.d8, self.n_classes, k_h=1, k_w=1, d_h=1, d_w=1, name='f_class_map', trainable=self.isTrain)

            return tf.nn.tanh(self.d8), self.d9
