...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        self.sar = tf.placeholder(tf.float32,
                                  [self.batch_size, 3*self.image_size, 3*self.image_size, 2*self.bands_sar],
                                   name='sar_images')
        self.opt= tf.placeholder(tf.float32,
                                 [self.batch_size, self.image_size, self.image_size, 2*self.output_c_dim],
                                 name='opt_images')
        self.opt_real_A = self.opt[:, :, :, :self.output_c_dim]

        # Edition for new experiments
        ############################# Uncomment for Original Experiments #############################
        # self.sar_resampling = tf.tanh(conv2d(self.sar, 2*self.bands_sar, d_h=3, d_w=3, name='sar_conv_resampling')) # uncomment for original experiments
        # self.real_A = tf.concat([self.sar_resampling, self.opt_real_A], 3)
        # self.real_B = self.opt[:, :, :, self.output_c_dim:2*self.output_c_dim]

        ############################# Uncomment for configuration for using just opt_t2  #####################
        # self.sarA = self.sar[:, :, :, 0:self.bands_sar]
        # self.sar_resampling = tf.tanh(conv2d(self.sarA, self.bands_sar, d_h=3, d_w=3, name='sar_conv_resampling'))
        # self.real_A = tf.concat([self.sar_resampling, self.opt_real_A], 3)
        # self.real_B = self.opt[:, :, :, self.output_c_dim:2*self.output_c_dim]

        ##############################  Uncomment for configuration for using just sar_t2  #####################
        self.sar_resampling = tf.tanh(conv2d(self.sar, 2*self.bands_sar, d_h=3, d_w=3, name='sar_conv_resampling'))
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        for epoch in xrange(args.epoch - 43):
#            data = glob('./datasets/{}/train/*.jpg'.format(self.dataset_name))
            data = glob.glob('/mnt/Data2/Old_Datasets_p2p/May052016May202017_multiresolution/train/*.npy')
    ...
    #                self.writer.add_summary(summary_str, counter)
                    ...
                    errD_fake = self.d_loss_fake.eval({ self.sar: sars, self.opt: opts })
                    errD_real = self.d_loss_real.eval({ self.sar: sars, self.opt: opts })
                    errG = self.g_loss.eval({ self.sar: sars, self.opt: opts })
