...
class pix2pix(object):
    ...
    def build_model(self):
        ...
        # Create seperated placeholder for sar and lansat image
        self.sar_data = tf.placeholder(tf.float32,
                                       [self.batch_size,
                                        3 * self.image_size,
                                        3 * self.image_size,
                                        self.input_c_dim],
        ...
        self.opt_data = tf.placeholder(tf.float32,
                                       [self.batch_size,
                                        self.image_size,
                                        self.image_size,
                                        self.output_c_dim],
        ...
        self.fake_opt_data = self.generator(self.sar_data)

        self.D, self.D_logits = self.discriminator(self.sar_data, self.opt_data, reuse=False)
        self.D_, self.D_logits_ = self.discriminator(self.sar_data, self.fake_opt_data, reuse=True)

        self.fake_opt_sample = self.sampler(self.sar_data)
...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        for epoch in xrange(args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                #  add if clouse to disable updating the classifier network model.
                # Update C network
                if isTrain is True:
                ...
                if np.mod(idx, 1) == 0:
                    ...
                    print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, c_loss Validation: %.8f" \
...
#                self.save(args.checkpoint_dir, counter)
            print("Mean Errors-->", np.mean(errorD), np.mean(errorG), np.mean(errC))
    ...
    def discriminator(self, sar_image, opt_image, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            sar_image_resampled = lrelu(conv2d(sar_image, self.input_c_dim, d_h=3, d_w=3, name='ds_conv'))
            image = tf.concat([sar_image_resampled, opt_image], 3)
    ...
    def generator(self, sar_image, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            image = conv2d(sar_image, self.input_c_dim, d_h=3, d_w=3, name='ds_conv')
            # e0 is (256 x 256 x sefl.gf_dim)
            e1 = self.g_bn_e2(conv2d(lrelu(image), self.gf_dim, name='g_e1_conv')) # 64x2x5x5+64 = 3,264
    ...
    def sampler(self, image, y=None):
        ...
        with tf.variable_scope("generator") as scope:
            ...
            image = conv2d(sar_image, self.input_c_dim, d_h=3, d_w=3, name='ds_conv')
            # e0 is (256 x 256 x sefl.gf_dim)
            e1 = self.g_bn_e1(conv2d(lrelu(image), self.gf_dim, name='g_e1_conv')) # 64x2x5x5+64 = 3,264
            # e1 is (128 x 128 x self.gf_dim)
            e2 = self.g_bn_e2(conv2d(lrelu(e1), self.gf_dim*2, name='g_e2_conv')) # (2x64)x64x5x5 + (2x64) = 204,928
            # e2 is (64 x 64 x self.gf_dim*2)
            e3 = self.g_bn_e3(conv2d(lrelu(e2), self.gf_dim*4, name='g_e3_conv')) # (4x64)x(2x64)x5x5 + (4x64) = 819,456
