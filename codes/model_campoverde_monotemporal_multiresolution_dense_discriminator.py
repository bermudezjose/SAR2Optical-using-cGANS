...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        sample_dir = os.path.join(sample_dir_root, self.dataset_name, 'samples')
        # sample_dir = sample_dir + 'samples/'
        if not os.path.exists(sample_dir):
            os.makedirs(sample_dir)

        data_list = glob.glob('/mnt/Data/Pix2Pix_datasets/Campo_Verde/Training/*.npy')
...
#                self.writer.add_summary(summary_str, counter)
    ...
    def discriminator(self, img_A, img_B, y=None, reuse=False):
        ...
        with tf.variable_scope("discriminator") as scope:
            ...
            return tf.nn.sigmoid(self.h3_d), self.h3_d
