...
#        self.d_sum = tf.summary.merge([self.d_sum, self.d_loss_real_sum, self.d_loss_sum])
        ...
        for epoch in xrange(args.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                # checar esto aqui
                sar_patch, opt_patch, trn_samples, trn_labels, labels_patch, idx_samples = load_data4ClassifierCV(self,
                                                                                                                  data_trn_list,
                ...
                _ = self.sess.run([g_optim],
                    feed_dict={self.real_data: batch_images, self.labels_classifier: trn_labels, self.dropout_keep_prob: 0.2, self.idx_samples: idx_samples})
                self.writer.add_summary(summary_str, counter)
                ...
                g_loss, g_lossl1, c_loss_fake, c_loss_real, out_sigmoid, labels1 = self.sess.run([self.g_loss0, self.g_lossl1, self.c_loss_Fake, self.c_loss_Real, self.C_Fake, self.labels_classifier],
                    feed_dict={self.real_data: batch_images, self.labels_classifier: trn_labels, self.dropout_keep_prob: 1.0, self.idx_samples: idx_samples})
                Gloss.append(g_loss)
