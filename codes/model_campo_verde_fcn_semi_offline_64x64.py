...
class pix2pix(object):
    def __init__(self, sess, image_size=256, load_size=286,
        ...
        self.fcn_bn_e5 = batch_norm(name='fcn_bn_e5')
        self.fcn_bn_e6 = batch_norm(name='fcn_bn_e6')
        self.fcn_bn_e7 = batch_norm(name='fcn_bn_e7')
        self.fcn_bn_e8 = batch_norm(name='fcn_bn_e8')

        self.fcn_bn_d1 = batch_norm(name='fcn_bn_d1')
        self.fcn_bn_d2 = batch_norm(name='fcn_bn_d2')
        self.fcn_bn_d3 = batch_norm(name='fcn_bn_d3')
        self.fcn_bn_d4 = batch_norm(name='fcn_bn_d4')
        self.fcn_bn_d5 = batch_norm(name='fcn_bn_d5')
        self.fcn_bn_d6 = batch_norm(name='fcn_bn_d6')
        self.fcn_bn_d7 = batch_norm(name='fcn_bn_d7')
...
#        self.fake_B_sum = tf.summary.image("fake_B", self.fake_B)
    ...
    def train_classifier(self, args):
        ...
        dataset_name = '05may2016_C01_synthesize_fcn_SEMI_OFFLINE_64x64/'
        ...
        tst_labels = np.array(data_Dic['labels'])[:self.image_size,:self.image_size]
        tst_img_A = np.array(data_Dic['img_A']).astype('float32')[:self.image_size,:self.image_size].reshape(1, self.image_size, self.image_size, self.input_c_dim)
        ...
        trn_labels = np.array(data_Dic['labels'])[:self.image_size,:self.image_size]
        trn_img_A = np.array(data_Dic['img_A']).astype('float32')[:self.image_size,:self.image_size].reshape(1, self.image_size, self.image_size, self.input_c_dim)
        ...
        # for epoch in xrange(1):
            ...
            # for idx in xrange(0, 1):
                ...
                for img in range(self.batch_size):
                    ...
                    Data = load_data4FCN_CV_cropping(self,
                                                     data_trn_list,
                    ...
                    beta.append(Data[3])
                if np.sum(beta) < self.batch_size:
                    continue
    ...
    def generator(self, image, y=None):
        with tf.variable_scope("generator") as scope:
            ...
            e1 = conv2dlayer(image, self.gf_dim, name='g_e1_conv') # 32x32x64
            e2 = self.g_bn_e2(conv2dlayer(lrelu(e1), self.gf_dim, d_h=1, d_w=1, name='g_e2_conv')) # 32x32x64
            e3 = self.g_bn_e3(conv2dlayer(lrelu(e2), self.gf_dim*2, d_h=2, d_w=2, name='g_e3_conv')) # 16x16x128
            e4 = self.g_bn_e4(conv2dlayer(lrelu(e3), self.gf_dim*2, d_h=1, d_w=1, name='g_e4_conv')) # 16x16x128
            e5 = self.g_bn_e5(conv2dlayer(lrelu(e4), self.gf_dim*4, d_h=2, d_w=2, name='g_e5_conv')) # 8x8x256
            e6 = self.g_bn_e6(conv2dlayer(lrelu(e5), self.gf_dim*4, d_h=1, d_w=1, name='g_e6_conv')) # 8x8x256
            e7 = self.g_bn_e7(conv2dlayer(lrelu(e6), self.gf_dim*8, d_h=2, d_w=2, name='g_e7_conv')) # 4x4x512
            e8 = self.g_bn_e8(conv2dlayer(lrelu(e7), self.gf_dim*8, d_h=1, d_w=1, name='g_e8_conv')) # 4x4x512

            self.d1= deconv2dlayer(tf.nn.relu(e8), self.gf_dim*8, d_h=1, d_w=1, name='g_d1')
            d1 = tf.layers.dropout(self.g_bn_d1(self.d1), 0.5, training=self.dropout)
            d1 = tf.concat([d1, e7], 3)

            self.d2 = deconv2dlayer(tf.nn.relu(d1), self.gf_dim*4, d_h=2, d_w=2, name='g_d2')
            d2 = tf.layers.dropout(self.g_bn_d2(self.d2), 0.5, training=self.dropout)
            d2 = tf.concat([d2, e6], 3)

            self.d3 = deconv2dlayer(tf.nn.relu(d2), self.gf_dim*4, d_h=1, d_w=1, name='g_d3')
            d3 = tf.layers.dropout(self.g_bn_d3(self.d3), 0.5, training=self.dropout)
            d3 = tf.concat([d3, e5], 3)

            self.d4 = deconv2dlayer(tf.nn.relu(d3), self.gf_dim*2, d_h=2, d_w=2, name='g_d4')
            d4 = tf.concat([self.g_bn_d4(self.d4), e4], 3)

            self.d5 = deconv2dlayer(tf.nn.relu(d4), self.gf_dim*2, d_h=1, d_w=1, name='g_d5')
            d5 = tf.concat([self.g_bn_d5(self.d5), e3], 3)

            self.d6 = deconv2dlayer(tf.nn.relu(d5), self.gf_dim, d_h=2, d_w=2, name='g_d6')
            d6 = tf.concat([self.g_bn_d6(self.d6), e2], 3)

            self.d7 = deconv2dlayer(tf.nn.relu(d6), self.gf_dim, d_h=1, d_w=1, name='g_d7')
            d7 = tf.concat([self.g_bn_d7(self.d7), e1], 3)

            self.d8 = deconv2dlayer(tf.nn.relu(d7), self.output_c_dim, d_h=2, d_w=2, name='g_d8')
...
# self.d9 = conv2dlayer(self.g_output, self.n_classes, k_h=1, k_w=1, d_h=1, d_w=1, name='g_class_map', trainable=self.isTrain)
    def classifier_fcn(self, image, reuse=False):
        with tf.variable_scope("classifier_fcn") as scope:
            ...
            e1 = conv2dlayer(image, self.gf_dim, name='fcn_e1_conv') # 32x32x64
            e2 = self.fcn_bn_e2(conv2dlayer(lrelu(e1), self.gf_dim*2, d_h=2, d_w=2, name='fcn_e2_conv')) # 16x16x128
            e3 = self.fcn_bn_e3(conv2dlayer(lrelu(e2), self.gf_dim*4, d_h=2, d_w=2, name='fcn_e3_conv')) # 8x8x256
            e4 = self.fcn_bn_e4(conv2dlayer(lrelu(e3), self.gf_dim*8, d_h=2, d_w=2, name='fcn_e4_conv')) # 4x4x512
            # e5 = self.fcn_bn_e5(conv2dlayer(lrelu(e4), self.gf_dim*8, d_h=2, d_w=2, name='fcn_e5_conv')) # 2x2x512
            # e6 = self.fcn_bn_e6(conv2dlayer(lrelu(e5), self.gf_dim*8, d_h=2, d_w=2, name='fcn_e6_conv')) # 1x1x512
            
            self.d1= deconv2dlayer(tf.nn.relu(e4), self.gf_dim*4, d_h=2, d_w=2, name='fcn_d1') # 8x8x256
            d1 = tf.layers.dropout(self.fcn_bn_d1(self.d1), 0.75, training=self.dropout)
            d1 = tf.concat([d1, e3], 3)

            self.d2 = deconv2dlayer(tf.nn.relu(d1), self.gf_dim*2, d_h=2, d_w=2, name='fcn_d2') # 16x16x128
            d2 = tf.layers.dropout(self.fcn_bn_d2(self.d2), 0.75, training=self.dropout)
            d2 = tf.concat([d2, e2], 3)

            self.d3 = deconv2dlayer(tf.nn.relu(d2), self.gf_dim*1, d_h=2, d_w=2, name='fcn_d3') # 32x32x64
            d3 = tf.layers.dropout(self.fcn_bn_d3(self.d3), 0.75, training=self.dropout)
            d3 = tf.concat([d3, e1], 3)

            self.d4 = deconv2dlayer(tf.nn.relu(d3), self.gf_dim*1, d_h=2, d_w=2, name='fcn_d4') # 64x64x128
            d4 = tf.layers.dropout(self.fcn_bn_d4(self.d4), 0.75, training=self.dropout)

            # self.d5 = deconv2dlayer(tf.nn.relu(d4), self.gf_dim*1, d_h=2, d_w=2, name='fcn_d5') # 32x32x64 
            # d5 = tf.concat([self.fcn_bn_d5(self.d5), e1], 3)

            self.class_map = conv2dlayer(tf.nn.relu(d4), self.n_classes, k_h=1, k_w=1, d_h=1, d_w=1, name='fcn_class_map') # 64x64x7
            # self.class_map = tf.layers.dropout(self.class_map, 0.5, training=self.dropout)
